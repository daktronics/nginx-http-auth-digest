...
static ngx_int_t ngx_http_auth_digest_handler(ngx_http_request_t *r)
{
  ...
  ngx_int_t rc;
  ngx_err_t err;
  ngx_str_t user_file, passwd_line, realm, localhost_nocheck;
  ngx_file_t file;
  ...
  if (ngx_strcmp(localhost_nocheck.data, "true") && ngx_http_auth_os_is_loopback(&(r->headers_in.server)) == 0))
    {
      return NGX_DECLINED;
    }
...
int ngx_http_auth_os_is_loopback(ngx_str_t *server)
{
