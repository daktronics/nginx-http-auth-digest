...
static char* ngx_http_auth_digest_merge_loc_conf(ngx_conf_t* cf, void* parent, void* child)
{
	...
	ngx_conf_merge_str_value(conf->localhost_nocheck, prev->localhost_nocheck, "false");
...
static ngx_int_t ngx_http_auth_digest_handler(ngx_http_request_t* r)
{
	...
	const int isLocalNoCheckEnabled = ngx_strcmp(alcf->localhost_nocheck.data, "true") == 0;

	if (isLocalNoCheckEnabled && isLocalHost) {
